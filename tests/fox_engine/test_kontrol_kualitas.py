...
def test_validasi_tugas_network_dengan_coroutine_salah(kontrol_kualitas):
    ...
    tugas = TugasFox(
        nama="tugas_network_invalid",
        mode=FoxMode.AUTO,
        coroutine_func=non_async_function,
...
def test_validasi_tugas_network_valid(kontrol_kualitas):
    ...
    tugas = TugasFox(
        nama="tugas_network_valid",
        mode=FoxMode.AUTO,
        coroutine_func=dummy_coroutine,
        jenis_operasi=IOType.NETWORK_GENERIC
    )
    try:
        kontrol_kualitas.validasi_tugas(tugas)
