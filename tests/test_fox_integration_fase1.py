...
@pytest.mark.asyncio
async def test_compiler_cache_is_not_used_in_fase1(setup_runtime):
    """Fase 1: Compiler cache harus tidak pernah digunakan."""
    runtime, _ = setup_runtime

    assert len(runtime.compiler_cache) == 0
    ...
    # Patch execute_compiled untuk memastikan tidak pernah dipanggil
    with patch.object(runtime, '_execute_compiled', new_callable=AsyncMock) as mock_compiled:
        await runtime.execute_function(fungsi, [])

        # Cache harus tetap kosong
        assert len(runtime.compiler_cache) == 0
        # Metode eksekusi terkompilasi tidak boleh dipanggil
